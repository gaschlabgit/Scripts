...
sub bowtie2 {
	...
	foreach my $file (@INfiles) {
		...
		# Run Bowtie2 
		my $btlog = `~/bin/bowtie2-2.2.4/bowtie2 -p 8 --phred33 -N 1 -x $REFERENCE -U $file -S $out.sam 2>>$OUTPUT_DIR/Bowtie_run.log`;
		...
		my $cleanSam = `java -Xmx4g -jar ~/bin/picard-tools-1.119/CleanSam.jar I=$out.sam O=$out.cleaned.sam`;
		...
		###    This will print out all the errors that it ignores (MAPQ errors)
		my $sortSam  = `java -Xmx4g -jar ~/bin/picard-tools-1.119/AddOrReplaceReadGroups.jar I=$out.cleaned.sam O=$out.final.sam SO=coordinate LB=$REFERENCE.fasta PL=ILLUMINA PU=unknown SM=$out VALIDATION_STRINGENCY=LENIENT`;
